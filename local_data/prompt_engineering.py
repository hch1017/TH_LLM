...
def info_dic_init():
    ...
    info_dic["id"] = ""
    info_dic["type"] = ""
    info_dic["from_human"] = "Now I am playing not-limited Texas Holder. The situation is "
    info_dic["from_gpt"] = ""
    name_dic = {}
...
def get_info_dic(info_dic, counter, line, name_dic, lose_hand, currency, pot):
    tmp_info_dic = info_dic.copy()
    tmp_info_dic["id"] += "_{}".format(counter)
    tmp_info_dic["type"] += type_determine(line.split()[1])
    tmp_info_dic["from_human"] += " The question is I am Seat " + get_seat(line, name_dic) + ", my hand is [" + lose_hand + "], the pot value is {}".format(currency) + str(pot) + ", I am a " + style + " guy. I can fold, bet, call, check, raise, and all-in. What should I do? If I bet or raise, then how much?"
    ...
    for fid, field in enumerate(fields):
        ...
        for i, line in enumerate(lines):
            ...
            if "button" in line and "Table" in line and "#" in line:
                ...
                pattern = r"\#(.*?)\ "
                match = re.search(pattern, line)
                info_dic["from_human"] += "Players are Seat " + match.group(1) + " is the button. "
                continue
        ...
        for i, line in enumerate(lines):
            ...
            if "posts small blind" in line:
                ...
                info_dic["id"] += "_0"
                info_dic["from_human"] += " PREFLOP Phase - " + name_position_switch(line, name_dic) + ". "
