...
# class LocalTH_V1(PromptRawDataset):

#     def __init__(self, output_path, seed, local_rank, dataset_name):
#         super().__init__(output_path, seed, local_rank, dataset_name)
#         self.dataset_name = "Local/TH_V1"
#         self.dataset_name_clean = "Local_TH_V1"
#         raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_V1', data_files='prompt.json')
#         self.raw_datasets = raw_data
#         # data_files = {"train":"TH_prompt.parquet", "test":"TH_prompt_test.parquet"}
#         # self.raw_datasets = load_dataset("parquet",
#         # data_dir='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/TH_prompt',data_files=data_files)
...
class LocalTH_entire_random(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_entire_random"
        self.dataset_name_clean = "Local_TH_entire_random"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_entire_random', data_files='prompt.json')
        self.raw_datasets = raw_data
    ...
    def get_prompt_and_chosen(self, sample):
        if sample['from_human'] is not None and sample['from_gpt'] is not None:
            return " Human: " + sample['from_human'] + " Assistant: " + sample[
                'from_gpt']
        return None

    def get_prompt_and_rejected(self, sample):
        if sample['from_human'] is not None and sample['reject'] is not None:
            return " Human: " + sample['from_human'] + " Assistant: " + sample[
                'reject']
        return None
    

class LocalTH_entire(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_entire"
        self.dataset_name_clean = "Local_TH_entire"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_entire', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_good(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_good"
        self.dataset_name_clean = "Local_TH_good"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_good', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad1(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad1"
        self.dataset_name_clean = "Local_TH_bad1"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad1', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad2(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad2"
        self.dataset_name_clean = "Local_TH_bad2"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad2', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad3(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad3"
        self.dataset_name_clean = "Local_TH_bad3"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad3', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad4(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad4"
        self.dataset_name_clean = "Local_TH_bad4"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad4', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad5(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad5"
        self.dataset_name_clean = "Local_TH_bad5"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad5', data_files='prompt.json')
        self.raw_datasets = raw_data
...
# Chinese dataset
class LocalTH_good_radical(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_good_radical"
        self.dataset_name_clean = "Local_TH_good_radical"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_good_radical', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad_radical(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad_radical"
        self.dataset_name_clean = "Local_TH_bad_radical"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad_radical', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_bad_conservative(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_bad_conservative"
        self.dataset_name_clean = "Local_TH_bad_conservative"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_bad_conservative', data_files='prompt.json')
        self.raw_datasets = raw_data
...
class LocalTH_good_conservative(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "Local/TH_good_conservative"
        self.dataset_name_clean = "Local_TH_good_conservative"
        raw_data = load_dataset(path='/root/DeepSpeedExamples/applications/DeepSpeed-Chat/training/step1_supervised_finetuning/Local/TH_good_conservative', data_files='prompt.json')
        self.raw_datasets = raw_data
